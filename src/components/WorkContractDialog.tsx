...
export const WorkContractDialog: React.FC<WorkContractDialogProps> = ({
  ...
  const fetchPersonalData = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('first_name, last_name, street_address, postal_code, city, signature_data_url')
        .eq('user_id', userId)
        .single();
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(14);
      doc.text('für kurzfristig beschäftigte Arbeitnehmer', 105, 30, { align: 'center' });
      ...
      doc.rect(20, yPos, 170, 30);
      yPos += 8;
      doc.text(`${personalData.first_name} ${personalData.last_name}`, 25, yPos);
      yPos += 6;
      ...
      yPos += 8;
      doc.text(`Das Arbeitsverhältnis beginnt am ${format(new Date(eventStartDate), 'dd.MM.yyyy', { locale: de })}`, 20, yPos);
      yPos += 6;
      doc.text(`und ist befristet bis zum ${format(new Date(eventEndDate), 'dd.MM.yyyy', { locale: de })}.`, 20, yPos);
      ...
      yPos += 8;
      doc.text(`Der Arbeitnehmer erhält eine Vergütung von ${contract.hourly_wage} Euro pro Stunde.`, 20, yPos);
      yPos += 6;
      doc.text('Die Vergütung ist jeweils zum Monatsletzten zur Zahlung fällig.', 20, yPos);
      
      yPos += 15;
      doc.text('§ 4 Arbeitszeit', 20, yPos);
      yPos += 8;
      const arbeitszeit = doc.splitTextToSize('Der Arbeitseinsatz erfolgt nach vorheriger Absprache mit dem Arbeitgeber. Während der Dauer dieses Vertrags wird eine maximale Arbeitszeit von 70 Tagen vereinbart.', 170);
      doc.text(arbeitszeit, 20, yPos);
      yPos += arbeitszeit.length * 6;
      
      // Continue with remaining sections...
      doc.save(`arbeitsvertrag_${personalData.first_name}_${personalData.last_name}_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setContract(prev => ({ ...prev, job_title: e.target.value }))}
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setContract(prev => ({ ...prev, hourly_wage: e.target.value }))}
                        placeholder="12.50"
                      />
                    </div>
                  </div>
                  <div>
                    <Label htmlFor="additional_agreements">Weitere Vereinbarungen</Label>
                    <Textarea
                      id="additional_agreements"
                      value={contract.additional_agreements}
                      onChange={(e) => setContract(prev => ({ ...prev, additional_agreements: e.target.value }))}
            ...
            <>
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => setUseExistingSignature(e.target.checked)}
                        />
              ...
              <div className="flex gap-2">
                <Button onClick={handleSignContract} disabled={isLoading}>
                  {isLoading ? 'Wird unterschrieben...' : 'Vertrag unterschreiben'}
                </Button>

...
const AnalyticsTab = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const eventRequestsOverTime = processTimeSeriesData(
        eventRequests || [],
        daysBack,
      );
      ...
      eventRequests?.forEach((req) => {
        ...
        if (req.photographer)
          serviceStats["Fotograf"] = (serviceStats["Fotograf"] || 0) + 1;
        if (req.videographer)
          serviceStats["Videograf"] = (serviceStats["Videograf"] || 0) + 1;
        if (req.light_operator)
          serviceStats["Lichtoperator"] =
            (serviceStats["Lichtoperator"] || 0) + 1;
      });
      ...
      // Process user growth
      const userGrowth = processTimeSeriesData(
        userRoles || [],
        daysBack,
        "users",
      );
  ...
  const calculateGrowthRate = (
    data: Array<{ count?: number; users?: number }>,
  ) => {
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {analytics.eventRequestsOverTime.reduce(
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {analytics.userGrowth.length > 0
                ? analytics.userGrowth[analytics.userGrowth.length - 1].users
                : 0}
            </div>
            <div className="flex items-center text-xs text-muted-foreground">
              {userGrowth >= 0 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Anzahl der eingehenden Anfragen pro Tag
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Am häufigsten angefragte Dienstleistungen
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Am häufigsten angefragte Musikrichtungen
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>

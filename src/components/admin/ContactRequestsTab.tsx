...
const ContactRequestsTab = () => {
  ...
  const fetchContactRequests = async () => {
    try {
      ...
      console.error("Error fetching contact requests:", error);
      trackError(
        error instanceof Error ? error : "Contact requests fetch failed",
        "data_fetch",
        "contact_requests_tab",
      );
    }
  };
            ...
            >
              ...
              <CardContent className="space-y-4">
                  ...
                  <div className="pt-4 border-t bg-muted/30 p-4 rounded-lg">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {request.responded_at &&
                        `Gesendet am: ${formatDistanceToNow(new Date(request.responded_at), { addSuffix: true, locale: de })}`}
                    </p>
                  </div>
                )}
                ...
                <div className="pt-4 border-t flex justify-end">
                  <Button
                    variant={
                      request.status === "geantwortet" ? "outline" : "default"
                    }
                    size="sm"

...
const QualificationRequestsTab = () => {
  ...
  const fetchQualificationRequests = async () => {
    try {
      ...
      setPendingRequests(
        requestsWithData.filter((req) => req.status === "pending"),
      );
  ...
  const handleReviewRequest = (request: QualificationRequest) => {
    ...
    if (
      ...
      request.qualifications?.validity_period_months
    ) {
      const expiryDate = new Date();
      expiryDate.setMonth(
        expiryDate.getMonth() + request.qualifications.validity_period_months,
      );
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (selectedRequest.qualifications?.is_expirable) {
        ...
        qualificationData.acquired_date = new Date()
          .toISOString()
          .split("T")[0];
      ...
      if (qualificationError) {
        console.error(
          "Error managing employee qualification:",
          qualificationError,
        );
        toast.error(
          "Qualifikation genehmigt, aber Zuordnung konnte nicht gespeichert werden",
        );
      } else {
        if (selectedRequest.qualifications?.is_expirable) {
          toast.success(
            "Qualifikation genehmigt und Gültigkeitszeitraum gesetzt!",
          );
        } else {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Bearbeiten Sie Qualifikationsanfragen von Mitarbeitern und verwalten
          Sie Ablaufdaten
        </p>
      </div>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        ...
                        <CardDescription>
                          Angefragt von {request.profiles?.first_name}{" "}
                      ...
                      <div className="space-y-2">
                        ...
                        {request.qualifications.proof_types && (
                          <div className="flex flex-wrap gap-1 mb-2">
                            {request.qualifications.proof_types.map(
                              (type, index) => (
                                <Badge
                                  key={index}
                                  variant="outline"
                        ...
                        {request.proof_files &&
                        request.proof_files.length > 0 ? (
                          <div className="space-y-2">
                            <p className="text-sm font-medium">
                              Hochgeladene Dateien:
                            </p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {request.proof_files.map(
                                (file: any, index: number) => (
                                  ...
                                  >
                                    ...
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        downloadProofFile(file.path, file.name)
                                      }
                      ...
                      <div className="p-3 bg-accent/20 border border-accent rounded-lg">
                        <div className="flex items-center gap-2 text-foreground">
                          ...
                          <span className="text-sm font-medium">
                            Ablaufende Qualifikation - Gültigkeitsdauer:{" "}
                            {request.qualifications.validity_period_months}{" "}
                            Monate
                          </span>
                        </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <CardDescription>
                          ...
                          {request.reviewed_by_profile && (
                            <>
                              {" "}
                              von {request.reviewed_by_profile.first_name}{" "}
                              {request.reviewed_by_profile.last_name}
                            </>
                          )}
                        </CardDescription>
                      </div>
          ...
          {expiringQualifications.length === 0 ? (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  Keine ablaufenden Qualifikationen
                </h3>
                <p className="text-muted-foreground">
            ...
            <div className="space-y-4">
              {expiringQualifications.map((qualification: any) => {
                const isExpiringSoon =
                  ...
                  isBefore(
                    new Date(qualification.expires_at),
                    addDays(new Date(), 30),
                  );
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          <CardDescription>
                            Mitarbeiter: {qualification.profiles?.first_name}{" "}
                            {qualification.profiles?.last_name}
                            <br />
                            Läuft ab am{" "}
                            {format(
                              new Date(qualification.expires_at),
            ...
            <div className="space-y-4">
              <div className="p-4 bg-muted rounded-lg">
                <h3 className="font-semibold">
                  {selectedRequest.qualifications?.name}
                </h3>
                <p className="text-sm text-muted-foreground">
                  Angefragt von {selectedRequest.profiles?.first_name}{" "}
              ...
              {selectedRequest.qualifications?.is_expirable && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Standard-Gültigkeit:{" "}
                    {selectedRequest.qualifications.validity_period_months}{" "}
                    Monate
                  </p>
                </div>
              )}

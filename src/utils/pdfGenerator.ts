...
export const generatePersonalDataPDF = (data: PersonalDataForPDF): jsPDF => {
  ...
  const labelValue = (label: string, value: string) => {
    ...
    const labelLines = pdf.splitTextToSize(label, contentWidth);
      ...
      labelLines.length * (labelSize * 0.5 + 1) + lineGap + textSize * 0.5 + 4,
    );
  ...
  if (data.birth_place || data.birth_country) {
    twoColumn(
      ...
      "Familienstand",
      formatLabel("marital_status", data.marital_status),
    );
  } else {
    twoColumn(
      "Familienstand",
  ...
  labelValue(
    "Adresse",
    `${data.street_address}, ${data.postal_code} ${data.city}`,
  );
  ...
  if (data.employment_type_other)
    labelValue("Sonstige Angabe", data.employment_type_other);

  const specialEmployment = [];
  if (data.is_marginal_employment)
    specialEmployment.push("Geringfügige Beschäftigung");
  if (data.has_other_employment || data.has_additional_employment)
    specialEmployment.push("Weitere Beschäftigung");
  if (specialEmployment.length > 0)
    labelValue("Besonderheiten", specialEmployment.join(", "));

  if (
    data.has_additional_employment &&
    Array.isArray(data.other_employment_details) &&
    data.other_employment_details.length > 0
  ) {
    ...
    data.other_employment_details.forEach((emp: any, idx: number) => {
      ...
      if (emp.start_date || emp.end_date) {
        const sd = emp.start_date
          ? (() => {
              const s = emp.start_date;
              const d = typeof s === "string" ? parseISO(s) : new Date(s);
              return isValid(d) ? format(d, "dd.MM.yyyy") : String(s);
            })()
          : "";
        const ed = emp.end_date
          ? (() => {
              const e = emp.end_date;
      ...
      if (emp.hours_per_week)
        parts.push(`Stunden/Woche: ${emp.hours_per_week}`);
      if (emp.monthly_income)
        parts.push(`Monatseinkommen: ${emp.monthly_income}€`);
  ...
  if (data.highest_school_degree || data.highest_professional_qualification) {
    ...
    if (data.highest_school_degree && data.highest_professional_qualification) {
      twoColumn(
        "Schulabschluss",
        formatLabel("school_degree", data.highest_school_degree),
        "Berufsqualifikation",
        formatLabel(
      ...
      if (data.highest_school_degree)
        labelValue(
          "Höchster Schulabschluss",
          formatLabel("school_degree", data.highest_school_degree),
        );
      if (data.highest_professional_qualification)
        labelValue(
  ...
  if (
    ...
    if (data.tax_id && data.tax_class_factor) {
      ...
      if (data.tax_class_factor)
        labelValue("Steuerklasse", data.tax_class_factor);
    }
    if (
      typeof data.child_allowances !== "undefined" &&
      data.religious_affiliation
    ) {
      twoColumn(
        "Kinderfreibeträge",
        String(data.child_allowances),
        "Konfession",
        data.religious_affiliation,
      );
    } else {
      if (typeof data.child_allowances !== "undefined")
        labelValue("Kinderfreibeträge", String(data.child_allowances));
      if (data.religious_affiliation)
        labelValue("Konfession", data.religious_affiliation);
    }
  }
  ...
  if (data.social_insurance_number || data.health_insurance_company) {
    ...
    if (data.social_insurance_number && data.health_insurance_company) {
      ...
      if (data.social_insurance_number)
        labelValue("Sozialversicherungsnummer", data.social_insurance_number);
      if (data.health_insurance_company)
        labelValue("Krankenkasse", data.health_insurance_company);
    }
  }
  ...
  if (data.signature_data_url) {
    ...
    try {
      pdf.addImage(data.signature_data_url, "PNG", margin, y, 80, 25);
    } catch {}
    y += 30;
...
export const generatePersonalDataPDFFileName = (
  data: PersonalDataForPDF,
): string => {

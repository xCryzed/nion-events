...
const MeineAngebote = () => {
  ...
  useEffect(() => {
    document.title =
      "Meine Angebote - DJ Aachen & Eventtechnik | NION Events Kundenbereich";
  }, []);
      ...
      if (isEmailVerified && user.email) {
        ...
        if (!emailError && emailRequests && emailRequests.length > 0) {
          ...
          if (!updateError) {
            ...
            const updatedRequests = emailRequests.map((req) => ({
              ...req,
              user_id: userId,
            }));
  ...
  const calculateTimeRemaining = (createdAt: string) => {
    ...
    const hoursElapsed =
      (currentTime.getTime() - created.getTime()) / (1000 * 60 * 60);
                    ...
                    <CardHeader>
                      <div className="flex flex-col space-y-3">
                        ...
                        {(request.status === "ANGEFRAGT" ||
                          ...
                          request.status === "RÜCKFRAGEN_OFFEN") &&
                          (() => {
                            ...
                            return (
                              <div className="flex items-center justify-between bg-muted/30 rounded-lg px-4 py-3 border-l-4 border-primary/20">
                                <div className="flex items-center space-x-3">
                                  <Timer
                                    className={`w-5 h-5 ${timeInfo.isUrgent ? "text-destructive" : timeInfo.expired ? "text-muted-foreground" : "text-primary"}`}
                                  />
                                  <div className="flex flex-col">
                                    ...
                                    <span
                                      className={`text-sm font-semibold ${timeInfo.isUrgent ? "text-destructive" : timeInfo.expired ? "text-muted-foreground" : "text-foreground"}`}
                                    >
                                      {timeInfo.display}
                                    </span>
                                  </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center text-sm">
                              ...
                              <span>
                                {request.guest_count === "Noch nicht bekannt"
                                  ? request.guest_count
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-1 text-sm">
                            {request.photographer && (
                              <span className="block">📷 Fotograf</span>
                            )}
                            {request.videographer && (
                              <span className="block">🎥 Videograf</span>
                            )}
                            {request.light_operator && (
                              <span className="block">💡 Lichtoperator</span>
                            )}
                            {request.dj_genres.length > 0 && (
                              <span className="block">
                                🎵 {request.dj_genres.join(", ")}
                              </span>
                            )}
                          </div>
                        </div>

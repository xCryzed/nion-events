...
const Auth = () => {
  ...
  const { isRegistrationEnabled, loading: registrationLoading } =
  ...
  useEffect(() => {
    document.title =
      "Anmelden - DJ Aachen & Eventtechnik | NION Events Kundenbereich";
  }, []);
    ...
    const {
      ...
      if (event === "SIGNED_IN" && session?.user && invitationToken) {
        ...
        toast({
          ...
          description:
            "Willkommen bei NION Events! Ihr Account wurde erfolgreich erstellt.",
        });
      ...
      if (session?.user && invitationToken && event !== "SIGNED_IN") {
        setTimeout(async () => {
          ...
          toast({
            ...
            description:
              "Sie wurden abgemeldet, damit Sie sich mit Ihrem neuen Mitarbeiterkonto registrieren können.",
            variant: "default",
          });
        }, 0);
        return;
  ...
  const validatePassword = (password: string): string[] => {
    ...
    if (/(.)\1{2,}/.test(password)) {
      errors.push(
        "Das Passwort darf nicht mehr als 2 gleiche Zeichen hintereinander enthalten",
      );
    }
  ...
  const handleEmployeeInvitation = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Ihr Mitarbeiterkonto wurde erstellt. Bitte überprüfen Sie Ihre E-Mails zur Bestätigung.",
      });
    } catch (error: any) {
    ...
    if (error) {
      ...
      if (error.message.includes("User already registered")) {
        toast({
          ...
          description:
            "Ein Benutzer mit dieser E-Mail-Adresse existiert bereits. Versuchen Sie sich anzumelden.",
          variant: "destructive",
        });
      } else {
        toast({
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="glass-card">
          <CardHeader className="text-center space-y-2">
            ...
            <CardDescription>
              {invitationToken
                ? "Vervollständigen Sie Ihre Mitarbeiter-Registrierung"
                : "Melden Sie sich an oder erstellen Sie ein neues Konto"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs
              defaultValue={invitationToken ? "signup" : "signin"}
              className="w-full"
            >
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="signin" disabled={!!invitationToken}>
                  Anmelden
                </TabsTrigger>
                <TabsTrigger value="signup">
                  {invitationToken ? "Registrierung" : "Registrieren"}
                </TabsTrigger>
              </TabsList>

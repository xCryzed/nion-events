...
import {
  generatePersonalDataPDFBlob,
  generatePersonalDataPDFFileName,
...
const personalDataSchema = z.object({
  ...
  iban: z
    .string()
    .regex(IBAN_REGEX, "Ungültige IBAN (Format: DE12 3456 7890 1234 5678 90)"),
  ...
  employment_type: z
    .array(z.string())
    .min(1, "Mindestens eine Beschäftigungsart auswählen"),
  employment_type_other: z.string().optional(),

  // Education
  highest_school_degree: z
    .enum([
      "abitur",
      "fachabitur",
      "mittlere_reife",
    ...
    .optional(),
  highest_professional_qualification: z
    .enum([
      "promotion",
      "diplom",
      "magister",
  ...
  social_insurance_number: z
    .string()
    .regex(SOCIAL_INSURANCE_REGEX, "Ungültige Sozialversicherungsnummer")
    .min(1, "Sozialversicherungsnummer ist erforderlich"),
...
const maritalStatusOptions = [
  ...
  {
    value: "eingetragene_lebenspartnerschaft",
    label: "Eingetragene Lebenspartnerschaft",
  },
];
...
const professionalQualificationOptions = [
  ...
  {
    value: "anerkannte_berufsausbildung",
...
const normalizeSchoolForDB = (v?: string) => {
  ...
  if (v === "hauptschule" || v === "volksschule")
    return "hauptschule_volksschule";
  return v;
};

const normalizeProfForDB = (v?: string) => {
  if (!v) return v as any;
  if (["diplom", "magister", "master", "staatsexamen"].includes(v))
    return "diplom_magister_master_staatsexamen";
  if (["meister", "techniker", "fachschule"].includes(v))
    return "meister_techniker_fachschule";
  return v;
};

const denormalizeSchoolForUI = (v?: string) => {
  switch (v) {
    case "abitur_fachabitur":
      return "abitur";
    case "hauptschule_volksschule":
      return "hauptschule";
    default:
      return v as any;
  }
...
const denormalizeProfForUI = (v?: string) => {
  switch (v) {
    case "diplom_magister_master_staatsexamen":
      return "master";
    case "meister_techniker_fachschule":
      return "meister";
    default:
      return v as any;
  }
...
const PersonaldatenStepper: React.FC<PersonaldatenStepperProps> = ({
  ...
  useEffect(() => {
    ...
    if (typeof v?.date_of_birth === "string" && v.date_of_birth) {
      form.setValue("date_of_birth", new Date(v.date_of_birth) as any, {
        shouldValidate: true,
        shouldDirty: false,
      });
    }
    if (typeof v?.start_date === "string" && v.start_date) {
      form.setValue("start_date", new Date(v.start_date) as any, {
        shouldValidate: true,
        shouldDirty: false,
      });
    }
    const sub = form.watch((values) => {
      ...
      if (typeof vv?.date_of_birth === "string" && vv.date_of_birth) {
        form.setValue("date_of_birth", new Date(vv.date_of_birth) as any, {
          shouldValidate: true,
        });
      }
      if (typeof vv?.start_date === "string" && vv.start_date) {
        form.setValue("start_date", new Date(vv.start_date) as any, {
          shouldValidate: true,
        });
      }
    });
  ...
  useEffect(() => {
    const updateCanvasWidth = () => {
      ...
      const containerWidth = Math.floor(
        signatureContainerRef.current.clientWidth,
      );
      ...
      if (user) {
        ...
        if (data) {
          ...
          const hasCurrentData =
            currentValues.first_name || currentValues.last_name;
          ...
          if (!hasCurrentData) {
            ...
            form.reset({
              ...
              has_additional_employment:
                data.has_additional_employment || false,
              other_employment_details: Array.isArray(
                data.other_employment_details,
              )
                ? (data.other_employment_details as any[])
                : [],
              has_other_employment: data.has_other_employment || false,
              is_marginal_employment: data.is_marginal_employment || false,
              highest_school_degree:
                denormalizeSchoolForUI(data.highest_school_degree as any) ||
                undefined,
              highest_professional_qualification:
                denormalizeProfForUI(
                  data.highest_professional_qualification as any,
                ) || undefined,
              tax_id: data.tax_id || "",
  ...
  const clearSignature = () => {
    if (signatureRef.current) {
      ...
      setSignatureDataUrl("");
      try {
        localStorage.removeItem("personal-data-signature");
      } catch {}
  ...
  const saveSignature = () => {
    ...
    try {
      localStorage.setItem("personal-data-signature", dataUrl);
    } catch {}
    ...
    const sanitize = (o: any) =>
      sortKeys({
        ...
        date_of_birth: o.date_of_birth
          ? new Date(o.date_of_birth as any).toISOString().slice(0, 10)
          : null,
        start_date: o.start_date
          ? new Date(o.start_date as any).toISOString().slice(0, 10)
          : null,
      });
  ...
  const onSubmit = async (data: PersonalDataFormData) => {
    ...
    try {
      const submitData: any = {
        ...
        highest_school_degree: normalizeSchoolForDB(
          data.highest_school_degree as any,
        ),
        highest_professional_qualification: normalizeProfForDB(
          data.highest_professional_qualification as any,
        ),
        tax_id: data.tax_id,
        tax_class_factor: data.tax_class_factor,
        ...
        if (hasChanges) {
          ...
          if (uploadError) {
            ...
            toast({
              title: "Erfolg",
              description:
                "Ihre Personaldaten wurden gespeichert, aber das PDF konnte nicht hochgeladen werden.",
            });
          } else {
            toast({
              title: "Erfolg",
              description:
                "Ihre Personaldaten wurden erfolgreich gespeichert und in die Personalakte hochgeladen.",
            });
          }
        } else {
          toast({
            title: "Gespeichert",
            description:
              "Keine Änderungen erkannt. Ihre Daten sind auf dem neuesten Stand.",
          });
        }
      } catch (pdfError) {
      ...
      switch (stepIndex) {
        ...
        case 3: { // Employment Information (+ optional additional employment)
          const baseOk =
            !!(
              values.start_date &&
              values.job_title &&
              Array.isArray(values.employment_type) &&
              values.employment_type.length > 0
            ) &&
            !errors.start_date &&
            !errors.job_title &&
          ...
          if (
            values.has_additional_employment &&
            (!values.other_employment_details ||
              values.other_employment_details.length === 0)
          ) {
        ...
        case 4: // Tax Information
          return (
            !!(
              values.tax_id &&
              values.tax_class_factor &&
              values.religious_affiliation
            ) &&
            !errors.tax_id &&
            !errors.tax_class_factor &&
            !errors.religious_affiliation &&
            !errors.child_allowances &&
  ...
  const handleComplete = () => {
    form.handleSubmit(onSubmit, () => {
      toast({
        title: "Bitte prüfen",
        description:
          "Bitte alle Pflichtfelder ausfüllen und Fehler korrigieren.",
        variant: "destructive",
      });
    })();
  };
  ...
  const AdditionalEmploymentManager = ({ field }: any) => {
    ...
    const isCurrentEmploymentValid =
      currentEmployment.company && currentEmployment.start_date;
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {(() => {
                      ...
                      if (!s) return "-";
                      const d =
                        typeof s === "string" ? parseISO(s) : new Date(s);
                    ...
                    {employment.end_date
                      ? (() => {
                          ...
                          const d2 =
                            typeof e === "string" ? parseISO(e) : new Date(e);
                          return isValid(d2)
                            ? format(d2, "dd.MM.yyyy")
                            : String(e);
                        })()
                      : "Aktuell"}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {employment.hours_per_week} Std/Woche,{" "}
                    {employment.monthly_income}€/Monat
                  </p>
                </div>
                <Button
                  type="button"
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter") {
                                      ...
                                      const base = current.filter(
                                        (t) =>
                                          !String(t).startsWith("Sonstige:"),
                                      );
                                      if (!base.includes("Sonstige"))
                                        base.push("Sonstige");
                                      base.push(`Sonstige: ${val}`);
                                      form.setValue("employment_type", base, {
                                        shouldValidate: true,
                                        shouldDirty: true,
                                      });
                                      otherField.onChange("");
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-start space-x-3 space-y-0">
              ...
              <div className="space-y-1 leading-none">
                <FormLabel>Ich übe weitere Beschäftigungen aus</FormLabel>
              </div>
            </FormItem>
          )}
        />
              ...
              <div className="flex items-center gap-2 min-h-[1.25rem]">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    <p>
                      Die 11-stellige Steuer-ID finden Sie in folgenden
                      Dokumenten: BZSt-Brief, Lohnsteuerbescheinigung,
                      Einkommensteuerbescheid oder in Ihrem ELSTER-Konto.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
            ...
            <FormItem>
              <div className="flex items-center gap-2 min-h-[1.25rem]">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    <p>
                      Pro Elternteil 0,5 Freibeträge. Zusammenlebende Eltern:
                      1,0 pro Kind. Getrennte Eltern: je 0,5 oder übertragbar
                      (1,0/0,0).
                    </p>
                  </TooltipContent>
                </Tooltip>
            ...
            <FormItem>
              <div className="flex items-center gap-2 min-h-[1.25rem]">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="max-w-xs">
                    <p>
                      Ihre Sozialversicherungsnummer finden Sie im
                      Sozialversicherungsausweis, DRV-Schreiben,
                      Krankenkassen-Dokumenten oder auf der Lohnabrechnung.
                    </p>
                  </TooltipContent>
                </Tooltip>
              </div>
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground mb-6">
          Bitte unterschreiben Sie im unten stehenden Feld zur Bestätigung Ihrer
          Angaben.
        </p>
      </div>
          ...
          <FormItem className="flex flex-row items-start space-x-3 space-y-0 p-4 border rounded-lg">
            ...
            <div className="space-y-1 leading-none">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                entsprechen. Ich verpflichte mich, meinem Arbeitgeber alle
                Änderungen, insbesondere in Bezug auf weitere Beschäftigungen
                (in Bezug auf Art, Dauer und Entgelt) unverzüglich mitzuteilen.
              </p>
              <FormMessage />
            </div>
          </FormItem>
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    onCancel ? onCancel() : navigate("/personaldaten")
                  }

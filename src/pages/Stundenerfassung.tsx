...
import { Clock, Calendar, CheckCircle, XCircle, AlertCircle, Briefcase, MapPin, Trash2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Stundenerfassung() {
  const [pastEvents, setPastEvents] = useState<(InternalEvent & { staff_category: string })[]>([]);
  ...
  useEffect(() => {
    document.title = 'Stundenerfassung - DJ Aachen & Eventtechnik | NION Events Mitarbeiterbereich';
    fetchPastEvents();
    fetchTimeRecords();
  }, [currentPage]);
    ...
    try {
      ...
      const pastEvents = registrations
        ...
        .filter(event => {
          ...
          const eventEnd = event.end_date ? parseISO(event.end_date) : parseISO(event.event_date);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const eventEnd = selectedEvent.end_date ? parseISO(selectedEvent.end_date) : new Date(eventStart.getTime() + 24 * 60 * 60 * 1000);
      
      const recordStart = parseISO(startDateTime);
      const recordEnd = parseISO(endDateTime);

      if (!isWithinInterval(recordStart, { start: eventStart, end: eventEnd }) ||
          !isWithinInterval(recordEnd, { start: eventStart, end: eventEnd })) {
        throw new Error('Arbeitszeiten müssen innerhalb des Event-Zeitraums liegen');
      }
      ...
      if (hasOverlap) {
        throw new Error('Die angegebenen Arbeitszeiten überschneiden sich mit einer bereits erfassten Arbeitszeit. Bitte wählen Sie einen anderen Zeitraum.');
      }
      ...
      toast({
        title: "Fehler",
        description: error.message || "Fehler beim Einreichen der Arbeitszeiten.",
        variant: "destructive",
      });
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteTimeRecord = async (recordId: string) => {
    if (!confirm('Möchten Sie diese Stundenaufzeichnung wirklich löschen?')) return;
    ...
    try {
      ...
      toast({
        title: "Fehler",
        description: error.message || "Fehler beim Löschen der Stundenaufzeichnung.",
        variant: "destructive",
      });
    } finally {
      setDeleting('');
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Select value={selectedEventId} onValueChange={handleEventChange}>
                ...
                <SelectContent className="bg-background border z-50">
                  {pastEvents.map(event => (
                    <SelectItem key={event.id} value={event.id} className="hover:bg-muted">
                      <div className="flex flex-col text-left">
                        <span className="font-medium">{event.title}</span>
                        <span className="text-xs text-muted-foreground">
                          {format(parseISO(event.event_date), 'dd.MM.yyyy', { locale: de })} • {event.location}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
              ...
              <Card className="bg-muted/20">
                <CardContent className="pt-4 space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <Badge variant="outline">{getSelectedEvent()?.staff_category || 'Nicht zugeordnet'}</Badge>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-primary" />
                      <span className="text-sm">{getSelectedEvent()?.location}</span>
                    </div>
                  </div>
                  ...
                  <div className="border-t pt-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <span className="text-sm font-medium text-muted-foreground">Event-Start:</span>
                        <p className="text-sm">
                          {format(parseISO(getSelectedEvent()!.event_date), 'dd.MM.yyyy, HH:mm', { locale: de })} Uhr
                        </p>
                      </div>
                      {getSelectedEvent()?.end_date && (
                        <div>
                          <span className="text-sm font-medium text-muted-foreground">Event-Ende:</span>
                          <p className="text-sm">
                            {format(parseISO(getSelectedEvent()!.end_date!), 'dd.MM.yyyy, HH:mm', { locale: de })} Uhr
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  {getSelectedEvent()?.end_date && (() => {
                    const eventStart = parseISO(getSelectedEvent()!.event_date);
                    const eventEnd = parseISO(getSelectedEvent()!.end_date!);
                        ...
                        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-lg p-3 mt-3">
                          <div className="flex items-start gap-2">
                            ...
                            <div className="text-sm">
                              <p className="font-medium text-amber-800 dark:text-amber-200">Mehrtägiges Event</p>
                              <p className="text-amber-700 dark:text-amber-300">
                                Bitte buchen Sie jeden Arbeitstag separat. Erstellen Sie für jeden Tag eine eigene Stundenerfassung.
                              </p>
                            </div>
                          </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:justify-self-start">
                <DateTimePicker
                  ...
                  minDate={getSelectedEvent() ? parseISO(getSelectedEvent()!.event_date) : undefined}
                  maxDate={getSelectedEvent() && getSelectedEvent()!.end_date 
                    ? parseISO(getSelectedEvent()!.end_date!) 
                    : getSelectedEvent() 
                      ? new Date(parseISO(getSelectedEvent()!.event_date).getTime() + 24 * 60 * 60 * 1000)
                      : undefined}
              ...
              <div className="md:justify-self-start">
                <DateTimePicker
                  ...
                  minDate={startDateTime ? parseISO(startDateTime) : undefined}
                  maxDate={getSelectedEvent() && getSelectedEvent()!.end_date 
                    ? parseISO(getSelectedEvent()!.end_date!) 
                    : getSelectedEvent() 
                      ? new Date(parseISO(getSelectedEvent()!.event_date).getTime() + 24 * 60 * 60 * 1000)
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Select value={breakMinutes.toString()} onValueChange={(value) => setBreakMinutes(Number(value))}>
                  ...
                  <SelectContent className="bg-background border z-50">
                    <SelectItem value="0" className="hover:bg-muted">0 Min (keine Pause)</SelectItem>
                    <SelectItem value="15" className="hover:bg-muted">15 Min</SelectItem>
                    <SelectItem value="30" className="hover:bg-muted">30 Min</SelectItem>
                    <SelectItem value="45" className="hover:bg-muted">45 Min</SelectItem>
                    <SelectItem value="60" className="hover:bg-muted">60 Min</SelectItem>
                    <SelectItem value="90" className="hover:bg-muted">90 Min</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="p-3 bg-primary/10 rounded-md border">
                    <p className="text-lg font-semibold text-primary">
                      {calculateWorkingHours(startDateTime, endDateTime, breakMinutes)} Stunden
                    </p>
            ...
            <Button 
              type="submit" 
              disabled={!selectedEventId || !startDateTime || !endDateTime || submitting}
              className="w-full"
              size="lg"
            >
              {submitting ? 'Wird eingereicht...' : 'Arbeitszeiten einreichen'}
            </Button>
          </form>
        </CardContent>
      </Card>
        ...
        <CardContent>
          {timeRecords.length === 0 ? (
            ...
            <div className="space-y-4">
              {(() => {
                ...
                return Object.entries(groupedRecords).map(([eventId, records]) => {
                  const sortedRecords = records.sort((a, b) => new Date(a.start_time).getTime() - new Date(b.start_time).getTime());
                  ...
                  const totalHours = records.reduce((sum, record) => {
                    return sum + parseFloat(calculateWorkingHours(record.start_time, record.end_time, record.break_minutes));
                  }, 0);
                  ...
                  return (
                    <div key={eventId} className="border rounded-lg p-4 space-y-3">
                      <div className="flex items-center justify-between flex-wrap gap-2">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Event: {format(parseISO(eventDate), 'dd.MM.yyyy', { locale: de })}
                            {records.length > 1 && (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {records[0].status === 'eingereicht' && (
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => handleDeleteTimeRecord(records[0].id)}
                      ...
                      {sortedRecords.map((record, index) => (
                        <div key={record.id} className={`${index > 0 ? 'border-t pt-3 mt-3' : ''}`}>
                          {records.length > 1 && (
                            <p className="text-xs text-muted-foreground mb-2">
                              Tag {index + 1} • {format(parseISO(record.start_time), 'dd.MM.yyyy', { locale: de })}
                            </p>
                          )}
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              ...
                              <p>{format(parseISO(record.start_time), 'dd.MM. HH:mm', { locale: de })}</p>
                            </div>
                            <div>
                              <span className="font-medium">Ende:</span>
                              <p>{format(parseISO(record.end_time), 'dd.MM. HH:mm', { locale: de })}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              <span className="font-medium">Arbeitszeit:</span>
                          ...
                          {records.length > 1 && record.status === 'eingereicht' && (
          ...
          <div className="flex items-center justify-between mt-6 pt-6 border-t">
            <div className="text-sm text-muted-foreground">
              {totalRecords > 0 ? (
                <>
                  Zeige {((currentPage - 1) * recordsPerPage) + 1} bis {Math.min(currentPage * recordsPerPage, totalRecords)} von {totalRecords} Aufzeichnungen
                </>
              ) : (

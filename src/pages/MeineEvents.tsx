...
const MeineEvents = () => {
  ...
  const fetchMyEvents = async (userId: string) => {
    try {
      ...
      trackError(error.message, "data_fetch", "meine_events", {
        user_id: userId,
      });
      toast({
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (reg) =>
          reg.internal_events.title
            .toLowerCase()
            .includes(searchTerm.toLowerCase()) ||
          reg.internal_events.location
            .toLowerCase()
            .includes(searchTerm.toLowerCase()),
      );
    }
    ...
    if (categoryFilter !== "alle") {
      filtered = filtered.filter(
        (reg) => reg.staff_category === categoryFilter,
      );
    }
    ...
    if (activeTab === "upcoming") {
      filtered = filtered.filter((reg) =>
        isAfter(new Date(reg.internal_events.event_date), now),
      );
    } else if (activeTab === "past") {
      filtered = filtered.filter((reg) =>
        isBefore(new Date(reg.internal_events.event_date), now),
      );
    }
  ...
  const upcomingEvents = registrations.filter((reg) =>
    isAfter(new Date(reg.internal_events.event_date), new Date()),
  );
  const pastEvents = registrations.filter((reg) =>
    isBefore(new Date(reg.internal_events.event_date), new Date()),
  );
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex flex-wrap items-start gap-3 mb-3">
                              ...
                              {getEventStatusBadge(
                                registration.internal_events.status,
                              )}
                              {getStatusBadge(registration.status)}
                              <Badge variant="outline">
                                {getCategoryLabel(registration.staff_category)}
                              </Badge>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-muted-foreground">
                              <div className="flex items-center gap-2">
                                ...
                                <span>
                                  {formatEventDate(
                                    ...
                                    registration.internal_events.end_date ||
                                      undefined,
                                  )}
                                </span>
                              </div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span>
                                  {registration.internal_events.location}
                                </span>
                              </div>
                            </div>
                            ...
                            {registration.internal_events.guest_count && (
                              <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground">
                                ...
                                <span>
                                  ca. {registration.internal_events.guest_count}{" "}
                                  Gäste
                                </span>
                              </div>
                            )}

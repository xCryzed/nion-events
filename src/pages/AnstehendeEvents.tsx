...
const AnstehendeEvents = () => {
  ...
  const registerForEvent = async (eventId: string, staffCategory: string) => {
    ...
    try {
      ...
      const existingRegistration = userRegistrations.find(
        (reg) => reg.event_id === eventId && reg.status !== "zurückgezogen",
      );
  ...
  const getRegistrationsForCategory = (category: string) => {
    return eventRegistrations.filter(
      (reg) =>
        reg.staff_category === category && reg.status !== "zurückgezogen",
    );
  };
          ...
          <div className="container mx-auto px-4">
            <Card className="max-w-md mx-auto text-center">
              <CardHeader>
                ...
                <CardDescription>
                  Sie benötigen Mitarbeiter-Rechte, um die anstehenden Events zu
                  sehen.
                </CardDescription>
              </CardHeader>
            </Card>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid gap-4">
                          {getPaginatedFilteredEvents().map((event, index) => {
                            const isUserRegisteredForEventAny =
                              isUserRegisteredForEvent(event.id);
                            const userEventRegistration =
                              getUserRegistrationForEvent(event.id);
                            const isEventInPast =
                              new Date(event.start) <= new Date();
                            const hasStaffRequirements =
                              event.extendedProps?.staffRequirements &&
                              event.extendedProps.staffRequirements.length > 0;
                            const availableSpots = hasStaffRequirements
                              ? event.extendedProps.staffRequirements.filter(
                                  (staff) =>
                                    staff.count - (staff.filled || 0) > 0,
                                )
                              : [];
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-muted">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setCurrentPage((prev) =>
                                    Math.min(getFilteredTotalPages(), prev + 1),
                                  )
                                }
                                disabled={
                                  currentPage === getFilteredTotalPages()
                                }
                              >
                                Weiter
                ...
                <div className="space-y-3">
                  ...
                  {selectedEvent.extendedProps?.guestCount && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>
                        {selectedEvent.extendedProps.guestCount} Gäste
                      </span>
                    </div>
                  )}
                </div>
                ...
                selectedEvent.extendedProps.staffRequirements.length > 0 && (
                  <div>
                    <h3 className="font-semibold mb-4">
                      Personal-Anforderungen & Anmeldung
                    </h3>
                    <div className="grid gap-4">
                      {selectedEvent.extendedProps.staffRequirements.map(
                        (staff, index) => {
                          const registrations = getRegistrationsForCategory(
                            staff.category,
                          );
                          const isUserRegisteredForEventAny =
                            isUserRegisteredForEvent(selectedEvent.id);
                          const userEventRegistration =
                            getUserRegistrationForEvent(selectedEvent.id);
                          const isRegisteredForThisCategory =
                            isUserRegisteredForCategory(
                              selectedEvent.id,
                              staff.category,
                            );
                            ...
                            <Card key={index} className="p-4">
                              <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <Badge
                                    variant={
                                      spotsLeft > 0 ? "outline" : "destructive"
                                    }
                                  >

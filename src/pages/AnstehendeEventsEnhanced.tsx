...
const AnstehendeEventsEnhanced = () => {
  ...
  const [contractStaffCategory, setContractStaffCategory] =
    useState<string>("");
  ...
  useEffect(() => {
    document.title =
      "Anstehende Events - DJ Aachen & Eventtechnik | NION Events Mitarbeiterbereich";
  }, []);
      ...
      const formattedEvents: Event[] = data.map((event) => {
        ...
        const getActualStatus = (
          ...
          const eventEnd = endDate
            ? new Date(endDate)
            : new Date(eventStart.getTime() + 4 * 60 * 60 * 1000); // Default 4h if no end time
            ...
            staffRequirements: (() => {
              ...
              if (event.staff_requirements) {
                if (typeof event.staff_requirements === "string") {
                  ...
                  staffReqs = Array.isArray(event.staff_requirements)
              ...
              if (event.qualification_requirements) {
                if (typeof event.qualification_requirements === "string") {
                  try {
                    ...
                    console.error(
                      "Error parsing qualification_requirements:",
                      e,
                    );
                    qualReqs = [];
                  }
  ...
  const canRegisterForPosition = (
    ...
    const categoryRequirement = requirements.find(
    ...
    const userQualificationNames = userQualifications
      .map((uq) => uq.qualifications?.name)
      .filter(Boolean);
  ...
  const getMissingQualifications = (
    requirements: any[],
    category: string,
  ): string[] => {
    if (!requirements || requirements.length === 0) {
    ...
    return requiredQualifications
      ...
      .map((reqQual: any) =>
        typeof reqQual === "string" ? reqQual : reqQual.name,
      );
  };
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              Diese Seite ist nur für Mitarbeiter zugänglich. Bitte melden Sie
              sich mit Ihrem Mitarbeiterkonto an.
            </p>
          </div>
        </div>
        <Footer />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex items-center gap-3 px-4 py-2 bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 rounded-full">
              ...
              <span className="text-sm text-muted-foreground font-medium hidden xs:inline">
                Events diesen Monat
              </span>
              <span className="text-sm text-muted-foreground font-medium xs:hidden">
                Events
              </span>
            </div>
                ...
                >
                      ...
                      <Badge
                        className={`text-white border-0 ${
                          event.extendedProps?.status === "geplant"
                            ? "bg-blue-500"
                            : event.extendedProps?.status === "laufend"
                              ? "bg-orange-500"
                              : event.extendedProps?.status === "abgeschlossen"
                                ? "bg-green-500"
                                : event.extendedProps?.status === "abgesagt"
                                  ? "bg-red-500"
                                  : "bg-purple-500"
                        }`}
                      >
                  ...
                  <CardContent>
                    ...
                    {event.extendedProps?.staffRequirements &&
                      event.extendedProps.staffRequirements.length > 0 && (
                        <div className="space-y-2">
                          <h4 className="font-semibold text-sm">
                            Personalanforderungen:
                          </h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                            {event.extendedProps.staffRequirements.map(
                              (requirement, index) => {
                                ...
                                const pricing = getPricing(
                                  event.extendedProps?.pricingStructure,
                                  requirement.category,
                                );
              ...
              <div className="flex items-center justify-between mt-6">
                <div className="text-sm text-muted-foreground">
                  ...
                  {Math.min(
                    ...
                    getTotalFilteredEvents(),
                  )}{" "}
                  von {getTotalFilteredEvents()} Events
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setCurrentPage(
                        Math.min(getFilteredTotalPages(), currentPage + 1),
                      )
                    }
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <Badge
                        className={`text-xs border-0 text-white ${
                          selectedEvent.extendedProps?.status === "geplant"
                            ? "bg-blue-500"
                            : selectedEvent.extendedProps?.status === "laufend"
                              ? "bg-orange-500"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="flex items-center gap-3 p-3 bg-background rounded-lg border">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-muted-foreground">
                                {selectedEvent.end
                                  ? `${format(new Date(selectedEvent.end), "HH:mm", { locale: de })} Uhr`
                                  : "-"}
                              </div>
                            </div>
                          </div>
                      ...
                      <CardContent className="p-4">
                        <div className="text-center space-y-2">
                          <div className="text-2xl font-bold">
                            {selectedEvent.extendedProps?.staffRequirements?.reduce(
                              (acc, req) => acc + (req.count || 0),
                              0,
                            ) || 0}
                          </div>
                          <div className="text-xs text-muted-foreground font-medium">
                            ...
                            .map((registration) => (
                              ...
                              >
                                ...
                                <span className="flex-1 truncate">
                                  {registration.profiles?.first_name}{" "}
                                  {registration.profiles?.last_name}
                                </span>
                                <Badge
                                  variant="outline"
                  ...
                  selectedEvent.extendedProps.staffRequirements.length > 0 && (
                    <div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                        {selectedEvent.extendedProps.staffRequirements.map(
                          (requirement, index) => {
                            const userRegistration = getUserRegistration(
                              selectedEvent.id,
                              requirement.category,
                            ...
                            const isFullyBooked =
                              filledCount >= requirement.count;
                            const canRegister = canRegisterForPosition(
                              selectedEvent.extendedProps
                                ?.qualificationRequirements || [],
                              requirement.category,
                            );
                            const missingQuals = getMissingQualifications(
                              selectedEvent.extendedProps
                                ?.qualificationRequirements || [],
                              requirement.category,
                            );
                            const pricing = getPricing(
                              selectedEvent.extendedProps?.pricingStructure,
                              requirement.category,
                            );
                            const isEditable = isEventEditable(
                              selectedEvent.extendedProps?.status || "",
                            );
                            ...
                            return (
                              <Card
                                key={index}
                                className="transition-all duration-200 hover:shadow-md border-2 hover:border-primary/20"
                              >
                                <CardHeader className="pb-3">
                                  ...
                                  <div className="w-full bg-muted rounded-full h-2">
                                    <div
                                      ...
                                      style={{
                                        width: `${Math.min((filledCount / requirement.count) * 100, 100)}%`,
                                      }}
                                    ></div>
                                  </div>
                                </CardHeader>

...
import { CheckCircle, Clock, X, Award, BookOpen, AlertCircle, Check, Upload, FileText, AlertTriangle, Calendar } from 'lucide-react';
...
const Qualifikationen = () => {
    ...
    const [qualificationRequests, setQualificationRequests] = useState<QualificationRequest[]>([]);
    ...
    useEffect(() => {
        document.title = 'Qualifikationen - DJ Aachen & Eventtechnik | NION Events Mitarbeiterbereich';
    }, []);
    ...
    const checkUserRoles = async (userId: string) => {
        try {
            ...
            if (employeeCheck.error || adminCheck.error) {
                console.error('Error checking roles:', employeeCheck.error || adminCheck.error);
                setIsEmployee(false);
                setIsAdmin(false);
            } else {
        ...
        try {
            ...
            if (qualificationIds.length > 0) {
                ...
                const requestsWithQuals = (data || []).map(request => ({
                    ...request,
                    qualifications: qualData?.find(q => q.id === request.qualification_id)
                }));
    ...
    const hasQualification = (qualificationId: string): boolean => {
        return userQualifications.some(uq => uq.qualification_id === qualificationId);
    };

    const hasPendingRequest = (qualificationId: string): boolean => {
        return qualificationRequests.some(req => 
            req.qualification_id === qualificationId && req.status === 'pending'
        );
    };

    const getRequestStatus = (qualificationId: string): QualificationRequest | null => {
        return qualificationRequests.find(req => req.qualification_id === qualificationId) || null;
    };
    ...
    const handleRequestQualification = async () => {
        ...
        if (selectedQualification.requires_proof && (!proofFiles || proofFiles.length === 0)) {
    ...
    const handleReviewRequest = async (requestId: string, status: 'approved' | 'rejected') => {
        try {
            ...
            toast.success(`Anfrage ${status === 'approved' ? 'genehmigt' : 'abgelehnt'}!`);
            setAdminNotes('');
            fetchAllRequests();
                ...
                <div className="container mx-auto px-4 py-8 pt-24 text-center">
                    <div className="max-w-md mx-auto">
                        ...
                        <p className="text-muted-foreground mb-6">
                            Sie benötigen Mitarbeiter-Rechte, um auf diese Seite zugreifen zu können.
                        </p>
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="container mx-auto px-4 py-8 pt-24">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <h1 className="text-3xl font-bold text-foreground">Qualifikationen</h1>
                    <div className="flex items-center gap-3 px-4 py-2 bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 rounded-full">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-primary text-lg">{userQualifications.length}</span>
                        </div>
                        <span className="text-sm text-muted-foreground font-medium">Qualifikationen</span>
                    </div>
                </div>
                ...
                <Tabs defaultValue="available" className="space-y-6">
                    <TabsList className="grid w-full grid-cols-3">
                        ...
                        <TabsTrigger value="my-qualifications" className="flex items-center gap-2">
                    ...
                    <TabsContent value="available" className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {qualifications.map((qualification) => {
                                ...
                                return (
                                    <Card key={qualification.id} className="transition-all hover:shadow-md">
                                        <CardHeader>
                                            ...
                                            {qualification.requires_proof && (
                                                <div className="mt-2 space-y-2">
                                                    ...
                                                    {qualification.proof_types && qualification.proof_types.length > 0 && (
                                                        <div className="flex flex-wrap gap-1">
                                                            {qualification.proof_types.map((type, index) => (
                                            ...
                                            {qualification.is_expirable && qualification.validity_period_months && (
                                                <div className="mt-2 flex items-center gap-2 text-sm text-blue-600">
                                                    <Calendar className="w-4 h-4" />
                                                    <span>Gültig für {qualification.validity_period_months} Monate</span>
                                                </div>
                                            )}
                                        </CardHeader>
                                        <CardContent>
                                            {!hasQual && !pendingRequest && requestStatus?.status !== 'rejected' ? (
                                                <Button
                                                    onClick={() => {
                                            ...
                                            ) : requestStatus?.status === 'rejected' && requestStatus.admin_notes ? (
                                                <div className="space-y-2">
                                                    <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                                                        <p className="text-sm text-red-800">
                                                            <strong>Ablehnungsgrund:</strong> {requestStatus.admin_notes}
                                                        </p>
                                                    </div>
                                                    <Button
                                                        variant="outline"
                                        ...
                                        <CardHeader>
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    ...
                                                    <CardDescription>
                                                        Angefragt am {format(new Date(request.requested_at), 'dd.MM.yyyy HH:mm', { locale: de })}
                                                    </CardDescription>
                                                </div>
                                                <Badge 
                                                    variant={
                                        ...
                                        <CardContent className="space-y-3">
                                            ...
                                            {request.reviewed_at && (
                                                <p className="text-xs text-muted-foreground">
                                                    Bearbeitet am {format(new Date(request.reviewed_at), 'dd.MM.yyyy HH:mm', { locale: de })}
                                                </p>
                                            )}
                                        </CardContent>
                                    </Card>
                        ...
                        {userQualifications.length === 0 ? (
                            <Card>
                                <CardContent className="p-8 text-center">
                                    ...
                                    <h3 className="text-lg font-semibold mb-2">Keine Qualifikationen</h3>
                                    <p className="text-muted-foreground">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {userQualifications.map((userQual) => {
                                    const isExpired = userQual.expires_at && isBefore(new Date(userQual.expires_at), new Date());
                                    const isExpiringSoon = userQual.expires_at && 
                                        isAfter(new Date(userQual.expires_at), new Date()) && 
                                        isBefore(new Date(userQual.expires_at), addDays(new Date(), 30));
                                    
                                    return (
                                        <Card key={userQual.id} className={`${
                                            ...
                                            <CardHeader>
                                                <div className="flex items-start justify-between">
                                                    <CardTitle className="text-lg">{userQual.qualifications?.name}</CardTitle>
                                            ...
                                            <CardContent className="space-y-2">
                                                <div className="text-sm text-muted-foreground">
                                                    {userQual.acquired_date && (
                                                        <p>Erworben am {format(new Date(userQual.acquired_date), 'dd.MM.yyyy', { locale: de })}</p>
                                                    )}
                                                    {userQual.expires_at && (
                                                        <p className={`${isExpired ? 'text-red-600 font-medium' : isExpiringSoon ? 'text-orange-600 font-medium' : ''}`}>
                                                            {isExpired ? 'Abgelaufen am' : 'Gültig bis'} {format(new Date(userQual.expires_at), 'dd.MM.yyyy', { locale: de })}
                                                        </p>
                                                    )}
                                                    {!userQual.expires_at && userQual.qualifications?.is_expirable === false && (
                                                        <p className="text-green-600">Unbegrenzt gültig</p>
                                                    )}
                                                </div>
                                                ...
                                                {isExpired && (
                                                    <Button 
                                                        ...
                                                        onClick={() => {
                                                            const qualification = qualifications.find(q => q.id === userQual.qualification_id);
                                                            if (qualification) {
                                                                setSelectedQualification(qualification);
                                                                setShowRequestDialog(true);
                                                            }
                    ...
                    <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
                        ...
                        <div className="space-y-4">
                            <div className="p-4 bg-muted rounded-lg">
                                ...
                                {selectedQualification?.is_expirable && selectedQualification?.validity_period_months && (
                                    <div className="mt-2 flex items-center gap-2 text-sm text-blue-600">
                                        <Calendar className="w-4 h-4" />
                                        <span>Gültig für {selectedQualification.validity_period_months} Monate</span>
                                    </div>
                                )}
                            </div>
                            ...
                            {selectedQualification?.requires_proof && (
                                <div className="space-y-3">
                                    <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                                        ...
                                        {selectedQualification.proof_types && selectedQualification.proof_types.length > 0 && (
                                            <div className="space-y-1">
                                                <p className="text-sm text-orange-700">Bitte laden Sie folgende Nachweise hoch:</p>
                                                <ul className="list-disc list-inside text-sm text-orange-600 ml-2">
                                                    {selectedQualification.proof_types.map((type, index) => (
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                            Erlaubte Formate: PDF, JPG, PNG, DOC, DOCX (max. 10MB pro Datei)
                                        </p>
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button
                                    ...
                                    className="flex-1"
                                    disabled={uploading || (selectedQualification?.requires_proof && proofFiles.length === 0)}
                                >
                                    {uploading ? (
                                        <>
